...
SPRING_BOOT_TYPE="spring_boot"
WILDFLY_TYPE="wildfly"
...
function usage {
  ...
  increment_print_tabs
    ...
    print "type         Give the type of the given app"
  decrement_print_tabs
  print
  ...
  increment_print_tabs
    ...
    print
    print "Spring Boot app options (default app type):"
    print "-p | --port"
    print "\tStart/Stop/Restart parameter. Server port on which the application should be launched/stopped. Will set the parameter 'server.port' on start or check running for isRunning command."
    print "-to | --timeout"
    print "\tStart/Stop/Restart parameter. Timeout in seconds. 90 seconds by default."
    print "-pf | --profile"
    ...
    print
    print "Wildfly app options (Use --wildfly in command line):"
    print "-c | --cmd"
    print "\tStart parameter. Start command as single string. This one is mandatory !"
    print "-po | --prot_offset"
    print "\tStart parameter. Port offset to be set in command line (override configuration)."
    print "-p | --port"
    print "\tStop/Restart parameter. Server port on which the application should be launched/stopped. Will set the parameter 'server.port' on start or check running for isRunning command."
    print "-to | --timeout"
    print "\tStart/Stop/Restart parameter. Timeout in seconds. 60 seconds by default."
  decrement_print_tabs
  print
}
...
function getAppType {
  tracePrint "Get type for app $appName"
  unset appType

  if [[ -z "$appName" ]]; then
    warningPrint "Try to get the type from an unknown app ..."
  else
    tracePrint "Try to get the jar file"
    local jar=`ls -a $appName | grep -E "$appName.*\\.jar"`
    if [[ ! -z $jar ]]; then
      tracePrint "Got jar $jar"
      appType=$SPRING_BOOT_TYPE
      return 0
    fi

    tracePrint "Try to find jboss or wildfly file(s)"
    local jbossCount=`find keycloak-server -name "*jboss*" | wc -l | tr -d [:space:]`
    local wildflyCount=`find keycloak-server -name "*wildfly*" | wc -l | tr -d [:space:]`
    tracePrint "Found $jbossCount jboss and $wildflyCount wildfly"
    if [[ jbossCount -ge 0 ]] || [[ jbossCount -ge 0 ]]; then
      appType=$WILDFLY_TYPE
      return 0
    fi
  fi
}
...
function parseStartCommandFromPid {
  ...
  if [[ $resCmdLine =~ "$DEPLOY_PARAMS_TIMEOUT" ]]; then
...
function listApps {
  ...
  if [[ "$details_option" = true ]]; then
    print "APP/TYPE/DEPLOYMENT_KEY/TOTAL_INSTANCES/PORTS"
  else
    print "APP/TYPE"
  fi
      ...
      if [[ "$details_option" = true ]]; then
        ...
        if [[ ! -z $deployKeysArr ]]; then
          ...
          do
            ...
            print "$appName/$appType/$dk/${#portArr[@]}/$ports"
          done
        else
          print "$appName/$appType///"
        fi
      else
        print "$app/$appType"
      fi
...
if [[ ! -z "$1" ]] && [[ $1 != -* ]]; then
  ...
  getAppType
  if [[ -z "$appType" ]]; then
    debugPrint "Cannot automatically find the type of app $appName ..."
  else
    debugPrint "Found automatically type $appType for app $appName"
  fi
  shift
fi
...
while [ "$1" != "" ]; do
  case $1 in
    --wildfly )
      debugPrint "=> Set Wildfly type"
      appType=WILDFLY_TYPE
      ;;
    --spring_boot )
      debugPrint "=> Set Spring Boot type"
      appType=SPRING_BOOT_TYPE
      ;;
    ...
    * )
      warningPrint "Unknown option $1. Please call $SCRIPT_NAME help for displaying possibilities."
      exit 1
  esac
  shift
done
...
if [[ ! -z "$appName" ]] && [[ -z "$appType" ]]; then
  errorPrint "Cannot find the type of app $appName ..."
  exit 1
fi
